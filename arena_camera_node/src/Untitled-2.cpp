...
void ArenaCameraNode::initialize_()
{
  ...
  */
  ...
  pub_qos_.durability(RMW_QOS_POLICY_DURABILITY_VOLATILE);
  // QoS history
                                      ...
                                      sensor_msgs::msg::Image& image_msg)
{
  try {
    ...
    // Step
    const size_t pixel_length_in_bytes = pImage->GetBitsPerPixel() / 8;
    const size_t width_length_in_bytes =
        image_msg.width * pixel_length_in_bytes;

    image_msg.step =
        static_cast<sensor_msgs::msg::Image::_step_type>(
            width_length_in_bytes);

    // Data
    const size_t image_data_length_in_bytes =
        width_length_in_bytes * image_msg.height;
    ...
    log_warn("Failed to create Image ROS MSG. Published Image Msg might be corrupted");
  }
}
...
void ArenaCameraNode::set_nodes_exposure_()
{
  ...
  // 🔥 SIEMPRE desactivar auto exposure
  Arena::SetNodeValue<GenICam::gcstring>(nodemap, "ExposureAuto", "Off");

  if (is_passed_exposure_time_) {
    // limitar al rango válido
    double min_exp = Arena::GetNodeValue<double>(nodemap, "ExposureTimeMin");
    double max_exp = Arena::GetNodeValue<double>(nodemap, "ExposureTimeMax");

    double exp = std::min(std::max(exposure_time_, min_exp), max_exp);

    Arena::SetNodeValue<double>(nodemap, "ExposureTime", exp);

    log_info(std::string("\tExposure set to ") + std::to_string(exp));
  }
}
